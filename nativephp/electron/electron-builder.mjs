...
const appWebsite = process.env.NATIVEPHP_APP_WEBSITE || appUrl;
...
export default {
    ...
    // DIGICLIP: explicit icon set. Without this, electron-builder falls back
    // to its default Electron icon (the missing taskbar/dock icon). The
    // build/icon.{png,ico,icns} files are copied from public/ by
    // InstallsAppIcon during `native:build` (see nativephp/electron/.gitignore).
    // Extension-less on purpose: builder picks .ico (win) / .icns (mac) /
    // .png set (linux) automatically.
    icon: 'build/icon',
    directories: {
    ...
    linux: {
        ...
        // mismatch orphans the running window from its .desktop icon in
        // docks/taskbars — pin it to the real WM_CLASS.
        desktop: {
            entry: {
    ...
    deb: {
        packageName: 'digiclip',
        maintainer: appAuthor,
        synopsis: 'Turn long videos into TikTok-ready vertical clips',
        description: 'DigiClip transcribes long-form video offline, picks the moments worth posting and renders captioned 9:16 clips.',
        icon: 'build/icon',
    },
    appImage: {
